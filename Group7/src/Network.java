...
public class Network {
	...
	private boolean needToWait1, needToWait2, needToWait3, needToWait4; //Allows the correct method to be triggered
	private int floor;
	...
	public synchronized FloorEvent schedToFloorSystem(FloorEvent floorEvent, int systemNumber)
	{
		...
		while(this.needToWait2 || this.needToWait3 || this.needToWait4)
		{
			try
			{
				wait();
	...
	{
		while(this.needToWait1 || this.needToWait3 || this.needToWait4)
		{
			try
			{
				wait();
	...
	{
		while(this.needToWait1 || this.needToWait2 || this.needToWait4)
		{
			try
			{
				wait();
	...
	{
		while(this.needToWait1 || this.needToWait2 || this.needToWait3)
		{
			try
			{
				wait();
